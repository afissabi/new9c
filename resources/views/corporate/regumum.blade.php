...
<script>
    ...
    function klinik() {
        ...
        $.ajax({
            url: "{{ url('corporate/get-reg-klinik') }}",
            type: "get",
    ...
    $('#pilihklinik').on('click', '.layanan', function (e) {
        ...
        $.ajax({
            url: "{{ url('corporate/get-reg-layanan') }}",
            type: "get",
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            data: {
                klinik : klinik,
    ...
    $('#pilihklinik').on('click', '.tanggal', function (e) {
        ...
        // $('#loading_tanggal').modal('show');
        $.ajax({
            url: "{{ url('corporate/get-reg-tanggal') }}",
            type: "get",
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            data: {
                klinik : klinik,
    ...
    $('#pilihjam').on('click', '.register', function (e) {
        ...
        // $('#loading_tanggal').modal('show');
        $.ajax({
            url: "{{ url('corporate/get-layanan-register') }}",
            type: "get",
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            data: {
                layanan : layanan,
    ...
    function pilihMetode() {
        $.ajax({
            url: "{{ url('corporate/get-metode-bayar-layanan') }}",
            type: "get",
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            data: {
                tipe : $('#tipe_bayar').val(),
            ...
            swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                        type:'POST',
                        url: "{{ url('corporate/simpan-register/layanan') }}",
                        data: formData,
                        contentType: false,
                        processData: false,
                    })
                    .done(function(hasil) {
                        ...
                        if (hasil.status == true) {
                            ...
                            swal.fire({
                                ...
                                window.location.href = `{{ url('corporate/thanks') }}`
                            });
                        } else {
                            title = "Gagal!";
                            icon = "error";
